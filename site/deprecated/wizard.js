...
function client(evt) {
  ...
  var clientData = JSON.stringify({
    ...
    q.open("POST", "http://localhost:3000/wizard/1", true);
  ...
  function ifexists(stored) {
    if (stored.status === "saved") {
      window.location.assign("http://localhost:3000/wizard/step2");
    }
    else {
      window.location.assign("http://localhost:3000/wizard/old");
    }
  }

  function prevpage(evt) {
    window.location.assign("http://localhost:3000/wizard/oldnew");
  }
